...
export default class LocationController implements IController {
    ...
    private initializeRoutes() {
        ...
        // this.router.patch(`${this.path}/:id`, [authMiddleware,
        // validationMiddleware(CreateLocationDto, true), roleCheckMiddleware(["admin"])], this.modifyUser);

        // this.router.delete(`${this.path}/:id`, [authMiddleware, roleCheckMiddleware(["admin"])], this.deleteUser);
    }

    private getAllLocations = async (req: Request, res: Response, next: NextFunction) => {
        try {
